package com.project.microservices.ToDoMicroservice.controllers;
...
import com.project.microservices.ToDoMicroservice.services.LoginService;
import com.project.microservices.ToDoMicroservice.services.ToDoService;
import com.project.microservices.ToDoMicroservice.utilities.JsonResponseBody;
import com.project.microservices.ToDoMicroservice.utilities.ToDoValidator;
...
public class RestController {
    ...
    public User giveMeAUser() {
        return new User("mark.anthony.80@gmail.com", "Mark Anthony", "HisPasswordEncrypted");
    }
    ...
    public ResponseEntity<JsonResponseBody> login(@RequestParam(value="email") String email, @RequestParam(value="password") String pwd){
        ...
        try {
            ...
            return ResponseEntity.status(OK).header("jwt", jwt).body(new JsonResponseBody(OK.value(), "Success! User logged in!"));
        }catch(UserNotInDatabaseException e1){
            return ResponseEntity.status(FORBIDDEN).body(new JsonResponseBody(FORBIDDEN.value(), "Forbidden: " + e1.toString()));
        }catch(UnsupportedEncodingException e2){
            return ResponseEntity.status(BAD_REQUEST).body(new JsonResponseBody(BAD_REQUEST.value(), "Bad Request: " + e2.toString()));
        }
    }
    ...
    public ResponseEntity<JsonResponseBody> showToDos(HttpServletRequest request){
        ...
        try {
            ...
            return ResponseEntity.status(OK).body(new JsonResponseBody(OK.value(), toDoService.getToDos((String) userData.get("email"))));
        }catch(UnsupportedEncodingException e1){
            return ResponseEntity.status(BAD_REQUEST).body(new JsonResponseBody(BAD_REQUEST.value(), "Bad Request: " + e1.toString()));
        }catch(UserNotLoggedException e2){
            return ResponseEntity.status(FORBIDDEN).body(new JsonResponseBody(FORBIDDEN.value(), "Forbidden: " + e2.toString()));
        }catch(ExpiredJwtException e3){
            return ResponseEntity.status(GATEWAY_TIMEOUT).body(new JsonResponseBody(GATEWAY_TIMEOUT.value(), "Session Expired: " + e3.toString()));
        }
    }
    ...
    public ResponseEntity<JsonResponseBody> newToDo(HttpServletRequest request, @Valid ToDo toDo, BindingResult result){
        ...
        if(result.hasErrors()){
            return  ResponseEntity.status(BAD_REQUEST).body(new JsonResponseBody(BAD_REQUEST.value(), "Data not valid: " + result.toString()));
        }

        try {
            loginService.verifyJwtAndGetData(request);
            return ResponseEntity.status(OK).body(new JsonResponseBody(OK.value(), toDoService.addToDo(toDo)));
        }catch(UnsupportedEncodingException e1){
            return ResponseEntity.status(BAD_REQUEST).body(new JsonResponseBody(BAD_REQUEST.value(), "Bad Request: " + e1.toString()));
        }catch(UserNotLoggedException e2){
            return ResponseEntity.status(FORBIDDEN).body(new JsonResponseBody(FORBIDDEN.value(), "Forbidden: " + e2.toString()));
    ...
    public ResponseEntity<JsonResponseBody> deleteToDo(HttpServletRequest request, @PathVariable(name="id") Integer toDoId){
        ...
        try{
            ...
            return ResponseEntity.status(OK).body(new JsonResponseBody(OK.value(), "ToDo correctly delete"));
        }catch(UnsupportedEncodingException e1){
            return ResponseEntity.status(BAD_REQUEST).body(new JsonResponseBody(BAD_REQUEST.value(), "Bad Request: " + e1.toString()));
        }catch(UserNotLoggedException e2){
            return ResponseEntity.status(FORBIDDEN).body(new JsonResponseBody(FORBIDDEN.value(), "Forbidden: " + e2.toString()));
